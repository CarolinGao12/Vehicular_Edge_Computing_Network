function [result_opti] = add_new_car_opti(Z,L_m,M,B_RSU,P_v,N_v,c_car,F_limit,buffer_number,N_right_add,N_left_add,V_postion_x_right_initial,V_postion_y_right_initial,V_postion_x_left_initial,V_postion_y_left_initial,v_speed,recy_time)
%UNTITLED5 Summary of this function goes here
Z_L_m=Z*L_m;

%   the car add to the direction right
V_positon_x_right_add= Z_L_m*ones(N_right_add,1);
...
V_postion_x_left_add=6*Z_L_m*ones(N_left_add,1);
...
for ii= 1: length(vehicle_position)
    if (vehicle_position(ii,1)>= Z_L_m) && (vehicle_position(ii,1)<= 6*Z_L_m)
